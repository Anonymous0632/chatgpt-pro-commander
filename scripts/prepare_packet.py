#!/usr/bin/env python3
"""Build a scanned source ZIP, Markdown bundle, and manifest for Pro review."""
...
class PacketError(RuntimeError):
    """Input scope or safety validation blocks attachment creation."""
...
def validate_root(root: Path) -> Path:
    ...
    if not resolved.is_dir():
        raise PacketError(f"Project root does not exist or is not a directory: {resolved}")
    home = Path.home().resolve()
    broad = {Path("/"), home, home / "Documents", home / "Downloads", home / "Desktop"}
    if resolved in broad:
        raise PacketError("Refusing an overly broad root; select one explicit project directory.")
    return resolved
...
def run_git(root: Path, args: list[str], *, check: bool = False) -> str | None:
    ...
    if result.returncode != 0:
        if check:
            ...
            raise PacketError(f"Git command failed: {detail}")
        return None
...
def git_metadata(root: Path) -> tuple[dict, Path | None, str]:
    ...
    if not is_within(root, top):
        raise PacketError("Unexpected relationship between project and Git roots.")
...
def git_candidates(root: Path, git_top: Path, scope: str) -> list[Path]:
    ...
    if result.returncode != 0:
        detail = result.stderr.decode("utf-8", errors="replace").strip()
        raise PacketError(f"Unable to list Git files: {detail}")
    paths: list[Path] = []
...
def explicit_candidates(root: Path, includes: list[str]) -> list[Path]:
    ...
    for raw in includes:
        ...
        if not is_within(lexical, root):
            raise PacketError(f"include is outside the project root: {raw}")
        if candidate.is_symlink():
            paths.append(candidate)
            continue
        resolved = candidate.resolve(strict=False)
        if not is_within(resolved, root):
            raise PacketError(f"resolved include is outside the project root: {raw}")
        if candidate.is_file():
            paths.append(candidate)
        elif candidate.is_dir():
            paths.extend(iter_directory(candidate))
        else:
            raise PacketError(f"include does not exist: {raw}")
    return paths
...
def build_packet(args: argparse.Namespace) -> dict:
    ...
    if not task_file.is_file() or task_file.is_symlink():
        raise PacketError("TASK file is missing, not a regular file, or is a symlink.")
    task_data = task_file.read_bytes()
    task_text = try_text(task_data)
    if task_text is None:
        raise PacketError("TASK file must be UTF-8 text.")

    output_dir = Path(args.output_dir).expanduser().resolve(strict=False)
    if is_within(output_dir, root):
        raise PacketError("Output directory must be outside the project root.")
    ...
    safety = {
        ...
        "findings": findings,
        "note": "The report records only type, relative path, and line; it never copies matched values.",
    }
    safety_path = output_dir / f"{args.task_id}-safety.json"
    safe_write_json(safety_path, safety)
    if not safety["ok"]:
        raise PacketError(f"Safety scan failed; see {safety_path.name}")
    ...
    bundle_lines = [
        "# ChatGPT Pro Commander evidence package",
        "",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--task-file", required=True, help="Path to UTF-8 TASK.md")
    parser.add_argument("--task-id", required=True, help="Current task UUID")
    parser.add_argument("--output-dir", required=True, help="Evidence directory outside the project")
    parser.add_argument("--include", action="append", default=[], help="Project-relative file or directory; repeatable")
    ...
    parser.add_argument("--fail-on-warnings", action="store_true", help="Treat ordinary sensitive-data warnings as blocking")
