#!/usr/bin/env python3
"""在浏览器发送前校验 PLAN/REVIEW 请求的协议、ID 和唯一回复标记。"""
...
def valid_uuid(value: object, field: str) -> str:
    if not isinstance(value, str):
        raise RequestError(f"{field} 必须是 UUID 字符串")
    try:
        return str(uuid.UUID(value))
    except ValueError as exc:
        raise RequestError(f"{field} 不是有效 UUID") from exc


def string_list(value: object, field: str, *, nonempty: bool = False) -> list[str]:
    if not isinstance(value, list) or any(not isinstance(item, str) for item in value):
        raise RequestError(f"{field} 必须是字符串数组")
    if nonempty and not value:
        raise RequestError(f"{field} 不能为空")
    return value
...
def extract_payload(text: str) -> dict:
    ...
    if not stripped.startswith("PRO_COMMANDER/1 REQUEST"):
        raise RequestError("请求必须以 PRO_COMMANDER/1 REQUEST 开头")
    matches = JSON_FENCE.findall(stripped)
    if len(matches) != 1:
        raise RequestError("请求必须包含且只包含一个 JSON fenced block")
    try:
        payload = json.loads(matches[0])
    except json.JSONDecodeError as exc:
        raise RequestError(f"请求 JSON 无效: {exc}") from exc
    if not isinstance(payload, dict):
        raise RequestError("请求 JSON 必须是 object")
    return payload
...
def validate_payload(
    ...
    *,
    ...
    if missing:
        raise RequestError(f"缺少字段: {', '.join(sorted(missing))}")
    if extra:
        raise RequestError(f"未知字段: {', '.join(sorted(extra))}")
    if payload["protocol"] != "PRO_COMMANDER/1":
        raise RequestError("protocol 不匹配")
