#!/usr/bin/env python3
"""Enable, pause, resume, disable, or query persistent commander state."""
...
def main() -> int:
    ...
    parser.add_argument("--state-file", help="Override the default state path, mainly for isolated tests")
    ...
    if args.json:
        ...
        print(f"ChatGPT Pro commander workflow: {state['status']}")
        print(f"State file: {path}")
        if state["source"] == "invalid_fail_closed":
            print("Invalid state file; safely defaulted to disabled.", file=sys.stderr)
    return 0
